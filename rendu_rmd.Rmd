...
for(i in 1:length(X)){
  for(j in 1:length(X)){
    ...
    A[i,j] = fonc.a(x = X[i], y = X[j], simga = sigma)
  }
}
...
On faire la même observation en repésentant graphiquement la fonction de compétition $a(x_i,x_j)$ :
```{r}
...
```
On peut voir que plus $\sigma$ est petit plus la fonction de compétition est étendu , donc plus la compétition est importante pour un interval de donné élevé.
